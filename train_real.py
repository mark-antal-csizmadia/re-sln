...
from data import get_data_real
from funcs import train_real, test, evaluate, get_lc_params
from utils import save_config

# arg parse
parser = argparse.ArgumentParser(description='re-sln training')
parser.add_argument('--dataset_name', type=str, choices=["animal-10n"], help='name of the dataset', required=True)
...
if __name__ == "__main__":
    ...
    model_name = "wrn-28-2" if args.dataset_name in ["cifar10", "cifar100", "animal-10n"] else "MODEL_NAME_FOR_CLOTHING1M"
    ...
    for n_epoch in range(1, args.n_epochs+1):
        ...
        if model_ema and 0 < args.lc_n_epoch and args.lc_n_epoch <= n_epoch:
            ...
            train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2)
        # train
        loss_epoch, accuracy_epoch = train_real(
            model=model, 
            device=device,
        ...
        writer.add_scalars('loss/train', {'all': loss_epoch}, n_epoch)
        # append to lists
